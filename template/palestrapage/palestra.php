<div class="post">
	<div class="grid-9 post-container">
		<div class="post-title" style="margin: auto;">
			<h2 class="padding-l-10 post-title-text">Palestras</h2>
		</div>
		<!-- <div class="post-content">
			<?php
			// query_posts('post_type=palestra');
			// while(have_posts()) : the_post(); ?>
			<div class="card">
				<div class="card-header">
					<?php // the_post_thumbnail(); ?>
				</div>
				<div class="card-body">
					<div class="card-body-item">
						<h2><a href="<?php // the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php // the_title_attribute(); ?>"><?php // the_title(); ?></a></h2>
					</div>
					<div class="card-body-item">
						<p><?php // the_content(); ?></p>
					</div>
				</div>
			</div>
			<?php
			// endwhile;
			// wp_reset_query(); ?>
		</div> -->
		<div class="post-content">
			<div class="card">
				<div class="card-header">
					<img src="<?php bloginfo('template_url'); ?>/design/layout/template/foto-palestra.png">
				</div>
				<div class="card-body">
					<div class="card-body-item">
						<h2><a href="profissao-carreira-e-protagonismo-na-area-da-quimica" rel="bookmark" title="Link para o curso">Curso de cromatografia líquida e gasosa</a></h2>
					</div>
					<div class="card-body-item">
						<div class="card-footer">
							<p></p>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="partner bg-blue-smoke padding-y">
	<div class="grid-12 padding-y partner-container">
		<div class="partner-title">
			<h1 class="partner-title-text">Associe-se e ganhe descontos nos cursos!</h1>
			<h1 class="partner-title-text">Veja algumas das vantagens de ser um associado da ABQRS</h1>
		</div>
		<div class="vantagem-slider">
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-idea"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Atualizações constantes</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-teacher"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Professores qualificados</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-network"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Networking com colegas</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-certificate"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Certificação, qualificação e premiação</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-equipment"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Equipamentos modernos e suporte diferenciado</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-discount"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Descontos em cursos e palestras</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-world"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Atuação em todo o estado do Rio Grande do Sul</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-group-idea"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Intercâmbio de ideias</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-book"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Material didático e extensivo</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-pc"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Inscrições online</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-group"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Atendimento especializado</p>
					</div>
				</div>
				<div class="vantagem-item">
					<div class="vantagem-item-logo">
						<i class="icon icon-payment"></i>
					</div>
					<div class="vantagem-item-title">
						<p>Pagamentos facilitados</p>
					</div>
				</div>
		</div>
	</div>
</div>
<div class="bg-blue-smoke padding-y row">
	<div class="grid-12 padding-y container-join-us">
		<div class="padding-y join-us-item">
			<div class="padding-y join-us-title">
				<p class="join-us-title-text">Junte-se a mais de 600 associados, faça parte da ABQRS!</p>
			</div>
		</div>
		<div class="padding-y join-us-item">
			<div class="padding-y content-button text-center">
				<a href="#junte-se" class="btn-primary" title="Associe-se agora!">ASSOCIE-SE AGORA!</a>
			</div>
		</div>
	</div>
</div>